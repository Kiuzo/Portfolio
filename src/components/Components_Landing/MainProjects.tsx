...
<div 
                // CHAVE PARA ALTURA UNIFORME: items-stretch + gap-y-8
                className="flex flex-col md:flex-row max-w-7xl w-full mt-10 gap-6 gap-y-8 md:gap-8 justify-center items-stretch px-4"
            >
                {projects.map((project) => (
                    <button
                        key={project.id}
                        onClick={() => setSelectedProject(project)}
                        // Adiciona h-full e flex-grow para o botão preencher o espaço
                        className={`
